#
# pi-nxengine-evo — NXEngine-evo as a bootable bare-metal Raspberry Pi image.
#
...
.PHONY: deps kernels rebuild verify netboot card clean-boards $(BOARDS)
...
# built against it. The worlds share one checkout of the LLVM tree libc++ is
# built from, but each builds its own copy of newlib, libc++ and Circle, so a
# machine with a small disk — a CI runner, most obviously — builds one board
# at a time and keeps only that board's world.
...
# the three kernels, boot configuration, and the engine's own data directory
# out of the upstream checkout. Cave Story's own files are not here and are
# not ours to ship — see the README for where they come from and what has to
# be done to them.
