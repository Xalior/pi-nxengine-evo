//
// circle_syscalls.cpp — NXEngine-evo's file access, routed to the core that
// owns the hardware.
//
// THE PROBLEM. NXEngine-evo reads its data files, its fonts and its saved
// games with plain C++: std::ifstream, opendir/readdir, stat(). Those reach
...
// interrupts included, belongs to core 0. NXEngine-evo has no file layer of
// its own to redirect, so there is nothing in the game to change.
//
...
// The game's own output — every line its logger writes, on descriptors 1
// and 2 — goes to the library's log ring instead: the application core
// formats the line and returns, and the hardware core prints it. The
